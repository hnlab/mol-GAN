...
class TrainModelRunner:
    ...
    def __init__(self, input_data_path, output_model_folder, n_epochs, 
                 starting_epoch, save_interval, warmup, lr_init, 
                 n_critic=5, batch_size=64, b1=0.5, b2=0.999, message=""):
        ...
        discriminator_name = "discriminator.txt" if self.starting_epoch == 1 else str(self.starting_epoch - 1) + "_discriminator.txt"
        discriminator_path = os.path.join(output_model_folder, discriminator_name)
        self.D = Discriminator.load(discriminator_path)

        generator_name = "generator.txt" if self.starting_epoch == 1 else str(self.starting_epoch - 1) + "_generator.txt"
        ...
        self.g_optimizer = torch.optim.AdamW(self.G.parameters(), lr=lr_rate, betas=(self.b1, self.b2))
        self.g_scheduler = torch.optim.lr_scheduler.LambdaLR(self.g_optimizer, lr_lambda=lambda1)
        
        self.d_optimizer = torch.optim.AdamW(self.D.parameters(), lr=lr_rate, betas=(self.b1, self.b2))
    ...
    def run(self):
        ...
        for epoch in range(self.starting_epoch, self.n_epochs + self.starting_epoch):
            ...
            self.lr_step()
            print("Gen learning rate：[{:.6f}]".format(self.g_optimizer.param_groups[0]['lr']))
            print("Disc learning rate：[{:.6f}]".format(self.d_optimizer.param_groups[0]['lr']))
            ...
            for i, real_mols in enumerate(tqdm(self.dataloader)):
                ...
                d_loss_1 = torch.mean(torch.nn.ReLU()(1.0 - (real_pred - torch.mean(fake_pred))))
                d_loss_2 = torch.mean(torch.nn.ReLU()(1.0 + (fake_pred - torch.mean(real_pred))))
                ...
                if i % self.n_critic == 0:
                    ...
                    g_loss_1 = torch.mean(torch.nn.ReLU()(1.0 + (real_pred - torch.mean(fake_pred))))
                    g_loss_2 = torch.mean(torch.nn.ReLU()(1.0 - (fake_pred - torch.mean(real_pred))))
                ...
                if i == len(self.dataloader) - 1:
                    if epoch % self.save_interval == 0:
                        generator_save_path = os.path.join(self.output_model_folder, str(epoch) + '_generator.txt')
                        discriminator_save_path = os.path.join(self.output_model_folder, str(epoch) + '_discriminator.txt')
...
class RunRunner:
    ...
    def __init__(self, encode, smiles_file, storage_path, n_epochs, save_interval,
                 starting_epoch, warmup, lr_init, latent_file="encoded_smiles.latent"):
    ...
    def run(self):
        ...
        if self.enc == 'yes':
            encode(smiles_file=self.smiles_file, output_latent_file_path=self.output_latent)
        ...
        T= TrainModelRunner(input_data_path=self.output_latent, output_model_folder=self.storage_path,
                            n_epochs=self.n_epochs, starting_epoch = self.starting_epoch,
                            save_interval = self.save_interval, warmup=self.warmup, lr_init=self.lr_init)
        T.run()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--smiles_file", help="The path to a data file.[json]", type=str)
