...
class TrainModelRunner:
    ...
    def run(self):
        ...
        print("Training of WGAN-div started.")
        ...
        for epoch in range(self.starting_epoch, self.n_epochs + self.starting_epoch):
            ...
            for i, real_mols in enumerate(tqdm(self.dataloader)):
                ...
                real_mols = real_mols.type(self.Tensor).requires_grad_()
                ...
                fake_mols = (
                    self.Sampler.sample(real_mols.shape[0])
                    .type(self.Tensor)
                    .requires_grad_()
                )
                ...
                real_grad_out = torch.ones(real_mols.shape[0], 1).type(self.Tensor)
                ...
                real_grad_norm = real_grad.view(real_grad.size(0), -1).pow(2).sum(
                    1
                ) ** (self.p / 2)

                fake_grad_out = torch.ones(fake_mols.shape[0], 1).type(self.Tensor)
                ...
                fake_grad_norm = fake_grad.view(fake_grad.size(0), -1).pow(2).sum(
                    1
                ) ** (self.p / 2)
